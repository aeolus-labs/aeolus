...
Subcommands:
  install [--force] [--no-start]
                         Write the launchd plist (~/Library/LaunchAgents),
...
func installService(args []string) {
	...
	// so the daemon actually has something to load when we start it.
	cfg := defaultConfigPath()
	...
	// Most users want install + start in one shot, so do it. Anyone who
	// needs the two steps split apart can pass --no-start.
	startService()
	fmt.Println()
